...
try {
  const { result, ...httpResponse } = await apiController.createVoiceTwoFactor(accountId, body);
...
|  --- | --- | --- |
| 400 | client request error | [`InvalidRequestError`](#invalid-request) |
...
try {
  const { result, ...httpResponse } = await apiController.createMessagingTwoFactor(accountId, body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch(error) {
  if (error instanceof ApiError) {
...
try {
  const { result, ...httpResponse } = await apiController.createVerifyTwoFactor(accountId, body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch(error) {
  if (error instanceof ApiError) {
...
| Name | Type | Tags | Description |
|  --- | --- | --- | --- |
| `result` | `string` | Optional | An error message pertaining to what the issue could be |
