...
try {
  const { result, ...httpResponse } = await apiController.voiceTwoFactor(accountId, body);
...
try {
  const { result, ...httpResponse } = await apiController.messagingTwoFactor(accountId, body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch(error) {
  if (error instanceof ApiError) {
...
try {
  const { result, ...httpResponse } = await apiController.verifyTwoFactor(accountId, body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch(error) {
  if (error instanceof ApiError) {
